                    ...
                    <div class="row gx-4 gx-lg-5 justify-content-center">
                        <div class="col-md-10 col-lg-8 col-xl-7">
                            <div class="site-heading">
                                <h1 style="font-size: 3.5rem; font-style: italic;">Mentor List</h1>
            ...
            <form action="MainController" method="POST">
                ...
                <table class="table" style="width: 90%;padding-top: 50px;padding-bottom: 50px;;margin-left: 50px;box-shadow:-4px 8px 8px rgba(0,0,0,0.1), -8px 16px 16px rgba(0,0,0,0.1), -16px 32px 32px rgba(0,0,0,0.15), -32px 64px 64px rgba(0,0,0,0.25);border: solid 1px">
                    ...
                    <tbody style="background-color: #d9e4f5;
                        ...
                        <c:forEach var="listMentor" items="${requestScope.LISTMENTOR}" varStatus="counter">
                        <form action="MainController" method="POST">
                            <tr>
                                <td style="color: black;padding-left: 30px;">${counter.count}</td>
                                <td style="color: black;">
                                    ${listMentor.username}
                                    <input type="hidden" name="txtUsername" value="${listMentor.username}"/>
                                </td>
                                <td style="color: black;">${listMentor.fullname}</td>
                                <td style="color: black;">${listMentor.email}</td>
                                <td style="color: black;">${listMentor.phone}</td>
                                <td style="color: black;">${listMentor.createBy}</td>
                                <td style="color: black">
                                    <input type="hidden" name="txtFlag" value="${listMentor.delFlag}"/>
                                    <c:if test="${(listMentor.delFlag eq false)}">
                                        <a style="color: blue">Active</a>
                                    </c:if>
                                    <c:if test="${(listMentor.delFlag eq true)}">
                                ...
                                <td style="color: black;">
                                    <c:if test="${(listMentor.delFlag eq false)}">
                                        <button   style="background-color: #9e3645;color: white;width: 70px;border-radius: 5px;font-size: 18px"
                                                  type="submit" value="Ban Mentor" name="btnAction"
