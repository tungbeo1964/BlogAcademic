...
<html>
    ...
    <body style="background-color: #d9e4f5;
        ...
        <c:if test="${not empty sessionScope.USER}">
          ...
          background-image: linear-gradient(315deg, #d9e4f5 0%, #f5e3e6 74%);"  >
                        <tr>
                            <th style="color: black;padding-right: 30px;padding-left: 30px;font-size: large">No.</th>
                            <th style="color: black;font-size: large">Content Title</th>
                            <th style="color: black;font-size: large">Create on</th>
                            <th style="color: black;font-size: large">Create by</th>
                            <th style="color: black;font-size: large;padding-right: 30px;">Status</th>
                            <th style="color: black;font-size: large">Setting</th>   
                            <th style="color: black;font-size: large;padding-right: 100px;">View</th>
                        </tr>
                    </thead>
          ...
          background-image: linear-gradient(315deg, #d9e4f5 0%, #f5e3e6 74%);">
                        <c:forEach var="listWaitingBlog" items="${requestScope.LISTBLOG}" varStatus="counter">
                        <form action="MainController" method="POST">
                            <tr>
                                <td style="color: black;padding-left: 30px;font-size: medium">${counter.count}</td>
                            <input type="hidden" name="txtBlogId" value="${listWaitingBlog.id}"/>
                            <td style="color: black;font-size: medium">${listWaitingBlog.title}</td>
                            <input type="hidden" name="txtTitleBlog" value="${listWaitingBlog.title}"/>
                            <td style="color: black;font-size: medium">${listWaitingBlog.createOn}</td>
                            <td style="color: black;font-size: medium">
                                <c:forEach var="accountName" items="${requestScope.LISTACCOUNT}">
                                    <c:if test="${(accountName.id eq listWaitingBlog.createBy.id)}">
                                        <font style="font-size: medium" color="black"><c:out value="${accountName.username}"/></font>
                                    </c:if> 
                                </c:forEach>
                            </td>
                            <td style="color: black">    
                                <c:if test="${listWaitingBlog.status eq 1}">
                                    <font style="font-size: medium" color: black ><c:out value="Waiting"/></font>
                                </c:if>
                                <c:if test="${listWaitingBlog.status eq 3}">
                                    <font style="font-size: medium" color="#ff3333" ><c:out value="Unapprove"/></font>
                                </c:if>
                            </td>
                            <td style="color: black">
                                <select style="border-radius: 5px;font-size: medium; box-shadow:-4px 8px 8px rgba(0,0,0,0.1), -8px 16px 16px rgba(0,0,0,0.1), -16px 32px 32px rgba(0,0,0,0.15), -32px 64px 64px rgba(0,0,0,0.25);  "  name="cboSetStatus">
                                    <option style="text-align: left;text-align: center; ">Approve</option>
                                    <option style="text-align: left;text-align: center">Un-Approve</option>
                                </select>                              
                                <button   style=" border-radius: 5px;background-color: #ffffff;color: black;font-size: medium"
                                          type="submit" value="Change Status" name="btnAction"
                                          onclick="return confirm('Do you want to change ?')">
                                    Change
                                </button>
                            </td>
                            <td style="color: black">
                                <button   style="background-color: #ffffff;color: black;border-radius: 5px;font-size: medium"
                                          type="submit" value="View Blog Detail Mentor" name="btnAction">
                                    View Blog
                                </button>
                            </td>
                            </tr>
