...
public class AccountDAO implements Serializable {
    ...
    public List<AccountTempDTO> getAllMentor() throws Exception {
        ...
        try {
            conn = DBConnection.makeConnection();
            if (conn != null) {
                String sql = "SELECT a.username, a.full_name , a.del_flag , m.mentor_email, m.mentor_phone, m.created_by "
                        + "FROM Account a "
                ...
                while (rs.next()) {
                    ...
                    boolean flag = rs.getBoolean("del_flag");
                    AccountTempDTO dto = new AccountTempDTO(username, fullname, email, phone, createBy, flag);
                    result.add(dto);
                }
            }
    ...
    public List<AccountTempDTO> getAllStudent() throws Exception {
        ...
        try {
            conn = DBConnection.makeConnection();
            if (conn != null) {
                String sql = "SELECT a.username, a.full_name , a.del_flag , s.student_email, s.student_phone "
                        + "FROM Account a "
                ...
                while (rs.next()) {
                    ...
                    boolean flag = rs.getBoolean("del_flag");
                    AccountTempDTO dto = new AccountTempDTO(username, fullname, email, phone, flag);
                    result.add(dto);
                }
            }
    ...
    public boolean banAccount(String username, boolean flag) throws SQLException, NamingException {
        Connection conn = null;
        PreparedStatement pstm = null;
        ...
        try {
            ...
            if (conn != null) {
                String sql = "Update Account "
                        + "Set del_flag=? Where username=?";
                pstm = conn.prepareStatement(sql);
                pstm.setBoolean(1, flag);
                pstm.setString(2, username);
    ...
    public boolean checkAccount(String username, String mail)
        ...
        try {
            conn = DBConnection.makeConnection();
            if (conn != null) {
                String sql = "Select acc.id, acc.username, stu.student_email, men.mentor_email from Account acc " 
                            + "left outer join Student stu on acc.id = stu.id " 
                            + "left outer join Mentor men on acc.id = men.id " 
                            + "where acc.username=? and stu.student_email=? OR acc.username=? and men.mentor_email=?";
                pstm = conn.prepareStatement(sql);
